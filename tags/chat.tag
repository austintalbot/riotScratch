<chat>
    <div>
        <div >{name}</div>
        <div show={ this.error_state=='error' } class={ error}> error: {error_display}</div>
        <div >
            <form name='sign_in_form' onsubmit={login}>
                <input type='email' required value='' ref='email'>
                <input type='password' required value='' ref='password'>
                <br/>
                <button disabled={this.state=='authenticated' }> sign In </button>
                <button disabled={ this.state=='unauthenticated' } onclick={signOut}> Sign Out</button>
                <button disabled={ this.state=='authenticated' } onclick={register}> register </button>
            </form>
        </div>
        <div show={ this.state=='authenticated' }>
            <form onsubmit={submit_message}>
                <input disabled type='email' placeholder='User Name' ref='logged_in_email' value=''>
                <input type='text' placeholder='Message' ref='logged_message' value=''>
                <button disabled={ this.state=='unauthenticated' } type='submit' value='Submit'> submit </button>
            </form>
        </div>
    </div>
    <hr/>
    <div class='messages'>
        <ul>
            <li href="#{message}" each={message in outputMessages}> {message}</li>
        </ul
    </div>
   ...
   this.outputMessages = [];
    ...
    firebase.auth().onAuthStateChanged(function(user) {
        if (user) {
           ...
           uid = user.uid;
        //var messageQueueRef = firebase.database().ref('/data/userWriteable/messageQueue');  for reference next line    
    ...
    this.submit_message = function(e) {
        e.preventDefault();
        /* Grab user who sent message  */
        var email = this.refs.logged_in_email.value;
        /* Grab message and clear the input box  */
            ...
            var message = snap.val();
             ...
             snap.forEach(function(childSnapshot) {
                childSnapshot.forEach(function(grandChildSnap) {
                    ...
                    //console.log(childData.child.messageText);
